...
const FLIPPERZERO_RELEASE: &str = "0.77.1";
const BUILD_URL: &str = "https://update.flipperzero.one/builds/";
...
/// Fetch Flipper Zero SDK.
fn fetch_sdk(download_dir: impl AsRef<Utf8Path>) -> io::Result<Utf8PathBuf> {
    let filename = format!("flipper-z-f7-sdk-{FLIPPERZERO_RELEASE}.zip");
    let path = download_dir.as_ref().join(&filename);
    let sdk_dir = download_dir.as_ref().join("sdk");

    if !path.exists() {
        // Fetch SDK
        download(&path, &format!("{BUILD_URL}firmware/{FLIPPERZERO_RELEASE}/{filename}"))?;
    }
...
/// Fetch Flipper Zero toolchain.
fn fetch_toolchain(download_dir: impl AsRef<Utf8Path>) -> io::Result<Utf8PathBuf> {
    let filename = format!("flipper-z-f7-sdk-{FLIPPERZERO_RELEASE}.zip");
    let path = download_dir.as_ref().join(&filename);
    let toolchain_dir = download_dir.as_ref().join("sdk");

    Ok(toolchain_dir)
}

/// Download contents of URL to a path.
fn download(path: impl AsRef<Utf8Path>, url: &str) -> io::Result<()> {
...
fn main() {
    ...
    let sdk = fetch_sdk(out_dir).expect("failed to fetch Flipper Zero SDK");
